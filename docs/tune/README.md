...
| 4 | ENABLE_EXPERT_PARALLEL | true | **DONE — FAIL** (eval 83<89, 4 new fails, host mem leak -> memwatch kill 6GiB floor, KV -1.4%); rolled back, last-known-good unchanged (tune 5) |
...
| 6 | ep-true | e8afbb2 (+record) | PASS (boot) / **died mid-eval** (memwatch host-mem kill) | **83/100** (53P/8Pa/8F) | **FAIL — eval 83 < 89 gate, 4 new fails (TC-52/58/61/65), host avail decayed 8.3->5.85 GiB over 40 min (leak outside cgroup), KV -1.4%; rolled back to tune-5 config, sweep exhausted** | TASK-73.06 |
